...
async function placeOrder(e) {
  ...
  const order = {
    ...
    house_number: String(f.get("house_number") || "").trim(),
    street: String(f.get("street") || "").trim(),
    area: String(f.get("area") || "").trim(),
    landmark: String(f.get("landmark") || "").trim(),
    city: String(f.get("city") || "").trim(),
    pincode: String(f.get("pincode") || "").trim(),
    // Combined address is kept for WhatsApp/display compatibility.
    address: [f.get("house_number"), f.get("street"), f.get("area"), f.get("landmark")].filter(Boolean).join(", "),
    pin: String(f.get("pincode") || "").trim(),
    items: c.map(x => ({
      id: x.id,
      dbId: x.p.dbId ?? null,
      name: x.p.name,
